...
def generate_vwap_graph(dataframe):
    ...
    dataframe['VWAP'] = dataframe['Cumulative TPV'] / \
        dataframe['Cumulative Volume']
    ...
    # Save the graph
    plt.savefig('static/vwap_graph.png')
    # plt.show()
...
def generate_supertrend_graph(dataframe, period=7, multiplier=3):
    ...
    dataframe['supertrend'] = calculate_supertrend(
        dataframe, period=period, multiplier=multiplier)
    ...
    # Save the graph
    plt.savefig('static/graph_supertrend.png')
    # plt.show()
...
def generate_macd_graph(dataframe, short_window=12, long_window=26, signal_window=9):
    ...
    dataframe['macd_signal'] = dataframe['macd_line'].ewm(
    ...
    # Save the graph
    plt.savefig('static/macd_graph.png')
    # plt.show()
...
def generate_rsi_graph(dataframe, window=14):
    ...
    # Save the graph
    plt.savefig('static/rsi_graph.png')
    # plt.show()
